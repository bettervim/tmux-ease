...
module TmuxConfig = struct
  ...
  type t = {prefix: string list; binds: bind list} [@@deriving yojson]
end
...
module Printer = struct
  ...
  let print (config : TmuxConfig.t) =
    ...
    let binds = generate_binds config.binds in
    let _ = generate_prefix_bind config.prefix in
    let header = "# Generated by BetterTmux" in
    let content = append_lines header [binds] in
